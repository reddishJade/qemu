                                        ...
                                        abi_ulong align)
{
    ...
    while (1) {
        ...
        if (addr > end_addr) {
            ...
            prot |= page_get_flags(addr+TARGET_PAGE_SIZE);
#endif
            if (prot) {
    ...
    if (!(flags & MAP_FIXED)) {
        ...
        if (!(flags & MAP_ANONYMOUS)) {
            ...
            if((flags & MAP_TYPE) == MAP_SHARED && (host_prot & PROT_WRITE)) {
                p = mmap(g2h_untagged(start), len, host_prot, flags | MAP_FIXED, fd, host_offset);
            } else {
                if (!(host_prot & PROT_WRITE)) {
                    mprotect (g2h_untagged(start), host_len, host_prot | PROT_WRITE);
                }
                p = pread(fd, g2h_untagged(start), HOST_PAGE_ALIGN(len), host_offset);
                if (!(host_prot & PROT_WRITE)) {
                    mprotect (g2h_untagged(start), host_len, host_prot);
                }
            }
 #else
